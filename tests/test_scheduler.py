...
async def test_notify_parent_on_success(env):
    ...
    # No truncation field at all — daemon doesn't reshape payload.
...
@pytest.mark.asyncio
async def test_notify_passes_result_through_unchanged(env):
    """Daemon does not truncate or reshape child result; transport-only."""
    store, _bus, sch = env
    parent = store.create_actor(name="sup", scope_id=ROOT_SCOPE, backend="pi")
    child = store.create_actor(
        name="worker",
    ...
    big = "x" * 50_000
    await sch.on_turn_completed(turn["turn_id"], outcome=TurnOutcome.SUCCEEDED, result=big)
