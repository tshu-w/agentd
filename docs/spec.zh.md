...
- **公共事件（public event）**：跨越 daemon 边界进入持久化事件日志（`events` 表）和 EventBus、对外部 client（`agentd logs`、`agentd wait --progress`、channel）可见的事件。在此边界之前的内容（backend 原始记录、runtime 内部状态如 `last_result`、debug 日志）不算公共事件。
...
- **Backend 原始事件（raw backend event）**：backend CLI 在 stdout 上发出的 JSON 对象，使用 backend 自身私有格式（如 pi 的 `message_update.assistantMessageEvent`、codex 的 `item.completed`、claude 的 `assistant.content`）。Backend 原始事件是 adapter normalization 的输入；它们不得作为公共事件 payload 透传。
...
| `turn.progress` | 见下方 | 最小规范化进度（仅 canonical schema） |
...
这两个限制不可互换：前者防御未知 backend producer 的任意大输入，后者约束 agentd 自己的行分隔 transport（公共事件流 + aggregate RPC 响应）。
...
每个 adapter 必须将 backend 行为规范化为 canonical signals：`turn.started` / `turn.progress` / `turn.end` / checkpoint 更新。Adapter 可通过 `ParsedLine.result` 字段通知 runner 更新 `last_result`（runtime 内部状态，与 `event_type` 正交，不发布到公共事件流）；最终 result 通过 `turn.end` / `actor.wait` / `actor.status` 暴露。

Adapter 输出规则：

- Adapter 必须发出 §4 中定义的 canonical payload。Backend 原始事件对象不得作为公共事件 payload 透传，即使作为未知事件类型的 fallback 也不行。
- 公共事件 payload 不得包含 backend 原始对象、完整 tool output、完整 turn input、完整 final result，或可用 delta 替代的 cumulative partial content。
- 未映射的 backend 记录必须 (a) 静默 drop，或 (b) 仅在显式开启 debug logging 时写入 bounded/redacted runtime 诊断日志。它们不得出现在公共事件流中。
...
- Completion 检测：进程退出。Pi 在每个 assistant turn 结束后都会输出 `turn_end`（tool-use turn、reply turn……），单个 `turn_end` **不代表**对话结束。Adapter 使用每个 `turn_end` 更新 runtime 内部 `last_result`（提取最新 assistant 文本，通过 `ParsedLine.result` 字段传递，`event_type="log"`；`turn_end` 原始对象不得作为公共 payload 转发），让进程继续运行至 EOF。由 runner 通用的"无显式 `turn.end` → 进程退出时兜底合成"逻辑处理 completion。
...
- 完整 result 不在公共事件流 payload 中；channel 通过 `agentd wait` 命令的 RPC ok response 或 `actor.status include_result=true` 从权威存储拉取。
