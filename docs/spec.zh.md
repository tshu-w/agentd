...
| `turn.progress` | 见下方 | 实时进展（文本、思考、工具调用） |
...
每个 backend adapter 负责将 raw output 映射为规范化、有界的 progress 事件。Progress 用于实时观察（`logs --follow`、`ps --watch`、`wait --progress`），不用于重放 backend 原始协议或承载完整 turn artifact。Payload `type` 必须是以下之一：
...
- Backend 原始事件对象不得作为公共事件 payload 转发
...
- `trigger.add`：参数 `actor`，`schedule`（cron）/ `at`（一次性，ISO 8601）/ `in`（一次性，相对延时）/ `every`（固定间隔）四选一，`type`（默认 `message`），`payload`
- `trigger.ls`：可选按 actor 过滤
- `trigger.rm`：按 trigger_id 删除

Cron 格式：标准 5-field（minute hour day month weekday），不支持秒级和 year 字段。时区语义：schedule 按 daemon 进程本地时区解释（与系统 cron 行为一致）；内部存储的 `next_fire_at` 为 UTC。`at` 接受 ISO 8601（无时区偏移时按本地时区）且必须是未来时间。`in`/`every` 接受 `90s`、`15m`、`3h`、`1d`、`1h30m` 这样的时长；`every` 必须 >= trigger 检查间隔（10s）。一次性 trigger 触发后自动删除；周期性 trigger 持续存在直到 `trigger.rm`。
...
agentd 使用两个相互独立的 transport 大小限制：

| 限制 | 数值 | 边界 | 用途 |
|---|---|---|---|
| `BACKEND_INPUT_MAX` | 16 MiB | runtime ← backend stdout | 对不可信外部进程的 stdout 单行硬兜底。超出的行被丢弃并记录 warning（§11）。 |
| `AGENTD_FRAME_MAX` | 4 MiB | daemon socket / CLI stdout / channel 子进程 readline | agentd 内部行分隔协议的 frame 上限，统一适用于 daemon socket、CLI 输出和 channel 子进程读取。 |
...
| `trigger_id` | 主键 |
| `target_actor_id` | FK |
| `kind` | `cron`（周期性，cron 表达式）、`every`（周期性，固定间隔）、`at`（一次性；触发后删除） |
...
每个 adapter 必须将 backend 行为规范化为 canonical signals：`turn.started` / `turn.progress` / `turn.end` / checkpoint 更新。Adapter 可通过 `ParsedLine.result` 字段将 result 文本传给 runner；这是 runtime 内部信号，与 `event_type` 正交，不发布到公共事件流。最终 result 通过 `turn.end` / `actor.wait` / `actor.status` 暴露。

Adapter 输出规则：

- Adapter 必须为所有公共事件发出 §4 中定义的 canonical payload。
- Progress payload 是实时观察记录（例如 text/thinking delta 以及 tool 名称、参数、状态）。Progress payload 不得包含 backend 原始对象、完整 tool output、完整 turn input、完整 final result，或可用 delta 替代的 cumulative partial content。
- 未映射的 backend 记录会被丢弃；不得出现在公共事件流中。

Completion：`turn.end` 是唯一的 turn completion 信号。兜底：仅在没有显式终止事件到达时，根据 execution termination + terminal intent 合成 `turn.end`。
...
- Completion 检测：进程退出。Pi 在每个 assistant turn 结束后都会输出 `turn_end`（tool-use turn、reply turn……），单个 `turn_end` **不代表**对话结束。Adapter 通过 `ParsedLine.result` 提取最新 assistant 文本（`event_type="log"`），并让进程继续运行至 EOF；`turn_end` 原始对象不得作为公共 payload 转发。由 runner 通用的"无显式 `turn.end` → 进程退出时兜底合成"逻辑处理 completion。
...
| Backend 输出行超过 `BACKEND_INPUT_MAX`（16 MiB） | 丢弃该行，记录 warning 含 backend / turn / 丢弃字节数。如果因此缺少终止记录，turn 以 `error=backend_record_too_large` 失败（在 `exit_code != 0` / 超时路径上追加到原失败原因之后）。 |
...
- **出站**：监听 actor 进度/状态 → 推送平台适配的更新（typing indicator、进度、失败提示）

Adapter 负责平台特有逻辑（消息格式、长度限制、Markdown 兼容、权限校验）。agentd 不感知平台细节。

Channel readline 要求：

- 读取 agentd CLI 流式输出（如 `agentd wait --progress`、`agentd logs --follow`）的 channel 子进程必须显式设置 readline limit。asyncio 默认值（64 KiB）不足以作为 agentd-formatted stream 的通用上限。
- 推荐实现值：4 MiB（与 `AGENTD_FRAME_MAX` / 内置 `RpcClient` 对齐）。
- 按目标平台分片、附件化、格式化 result（如 Telegram 消息限制、文件附件）是 channel adapter 自己的责任。

结果交付由 channel 自行决定。Channel 可以转发最终 result，也可以让 agent 通过 channel skill 直接回复（Telegram 参考 adapter 采用这种方式）。
...
5. Turn 结束后按需报告失败/停止；成功回复由 agent 通过 Telegram skill 发送
