...
async def wait_for_actor(
    ...
    *,
    ...
    if last_payload is None:
        return WaitResult(ok=False, error="agentd wait exited without payload")

    state = (last_payload.get("actor") or {}).get("state", "")
    if state == "idle":
        return WaitResult(ok=True, result_text=str(last_payload.get("result") or ""))
    if state == "closed":
        error_msg = last_payload.get("error")
        if error_msg:
            return WaitResult(ok=False, error=str(error_msg))
        return WaitResult(ok=False, stopped=True)

    error = last_payload.get("error") or f"Actor {state or 'closed'}"
    return WaitResult(ok=False, error=str(error))
